...
**The tension:** Raw error details — stderr output, exception messages, internal paths — are essential for the mind's self-correction loop. If a skill fails, it needs to know why. Suppressing that information makes the agent less capable. But piping it back to the end user (e.g., Discord) exposes internal file paths, API key validation failures, and database schema details.
