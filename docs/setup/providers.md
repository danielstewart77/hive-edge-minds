...
- **Self-improvement** — new tools and skills can be created at runtime under `~/.claude/skills/` and become available in the same session, because the CLI re-reads its skill directory dynamically
...
Within Hive Mind, use a dedicated Ollama-backed mind (`cli_ollama` backend) for Ollama delegation rather than the standalone plugin.
