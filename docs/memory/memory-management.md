...
**Manual** — the user says "remember this" (a specific item or the current thread).
...
If any agent returns `FAIL`, the pipeline stops and the user is informed at which step and why. If all chunks are classified as `discard`, the pipeline exits cleanly after `route-memory` with no writes.
...
- Content covers the same topic or fact → surface to the user (Step 3).
...
If the user says something outside these options, interpret intent and confirm before acting.
...
No matches → proceed to write. Every node must have at least one edge — if no relationship can be established from the chunk, surface this to the user before creating an isolated node.

One clear match → upsert using graph_upsert. Update properties per the data class spec.

Multiple possible matches → do not write. Surface to the user (Step 3).

## Step 3 — Prompt the user (multiple matches only)

Present the incoming entity and candidate matches. Suggest the most likely match, let the user decide:
...
Call graph_upsert with entity type, name, properties, and relationship fields per data class spec and the user's decision.
...
---
name: notify-action
description: Procedure for handling a memory chunk with the notify action. Use when save-memory encounters a chunk that requests a future alert. Determines whether to call set_reminder, add a scheduler entry, or inform the user the feature is unsupported.
user-invocable: false
---
...
Edge case — time already past: discard the notify action. Inform the user it was skipped and why.
...
3. Notify the user: this is added and requires a container restart to take effect.
...
→ Feature does not exist yet. Tell the user:
...
description: Creates a new data class spec file in specs/data-classes/ and registers it in the index. Use when classify-memory encounters content that doesn't fit any existing class, or when the user requests a new class directly.
user-invocable: true
---
...
If the proposed name is too close to an existing one, flag it and ask the user to choose a different name before proceeding.
...
Good: "A named individual — recognizable by a person's name as the subject, with associated facts about their role, relationship to the user, or preferences."
...
Spec files are read by the mind, not humans. Write minimally. No padding, no preamble. Every line must carry information.
```
...
Follow the spec exactly: run searches before writing, surface conflicts to the user, wait for decisions before acting.
