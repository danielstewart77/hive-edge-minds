...
- [ ] Simulating a mind-session timeout produces the expected 6-line trace (see spec below)

---

## Goal

Give operators (and the mind itself) enough signal to diagnose failures without drowning
in noise. A mind-session timeout is undiagnosable when the gateway emits zero log lines
during the incident. This spec closes that gap.
...
## What a Mind-Session Timeout Looks Like

```
[INFO]  forward_to_mind: start mind=example group=44b2bf73
[INFO]  forward_to_mind: using session=618ddc0a mind=example
[INFO]  message: session=618ddc0a chars=312
[INFO]  send_message: start session=618ddc0a mind=example
[INFO]  send_message: spawning session=618ddc0a mind=example model=gpt-oss:20b-32k
[WARNING] send_message: slow response session=618ddc0a mind=example elapsed=90.0s
[ERROR]  forward_to_mind: timeout mind=example after 120.0s
```
