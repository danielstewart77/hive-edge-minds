...
app = FastAPI(title="Hive Mind Voice Server", docs_url=None, redoc_url=None, openapi_url=None)
...
async def tts(req: TTSRequest):
    ...
    if ref_path is None:
        ...
        # silently reuses the previous caller's voice instead of falling back
        # to a true default. Fail loud at the boundary so cross-mind voice
        # bleed is impossible and missing voice_refs are visible in logs.
...
async def wake_word_config(req: WakeWordConfigRequest):
    try:
        config = _wake_word.update_config(
            phrases=req.phrases,
            cancel_phrases=req.cancel_phrases,
            followup_window_seconds=req.followup_window_seconds,
            min_command_chars=req.min_command_chars,
        )
...
async def wake_word_detect(req: WakeWordDetectRequest):
    result = _wake_word.process_transcript(req.transcript)
...
async def health():
    return {
        ...
        "wake_word": _wake_word.get_status(),
    }
