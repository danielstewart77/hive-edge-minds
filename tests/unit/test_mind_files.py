...
One test per numbered requirement, named for the requirement it proves.
Requirement 6 — an unreachable mind says so rather than showing an empty
folder — is the console's to prove and lives in the hive repo, because only
a caller can observe a mind that does not answer.
...
the real code compute the roots, exactly as `test_skills_sync` does.
"""
...
os.environ.setdefault("CLAUDE_CONFIG_DIR", tempfile.mkdtemp(prefix="files-test-"))

import mind_files  # noqa: E402
...
def _build(monkeypatch, tmp_path, harness: str):
    ...
    `claude_home` and `codex_home` are separate directories on purpose:
    a test that pointed both environment variables at one path could not
    tell "reads the codex location" from "reads whatever is there".
    """
...
def test_a_text_file_is_not_called_binary_by_where_the_sniff_lands(mind):
    ...
    """
    client, home, _ = mind
    body = "x" * (mind_files.SNIFF_BYTES - 1) + "—" + "tail of the hook\n"
