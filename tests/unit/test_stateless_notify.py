...
SCRIPT_PATH = str(__import__("pathlib").Path(__file__).resolve().parents[2] / "tools/stateless/notify/notify.py")
...
class TestStatelessNotify:
    ...
    def test_notify_missing_credentials(self):
        ...
        env = os.environ.copy()
        # Clear credential vars so the script can't accidentally send a real Telegram message.
